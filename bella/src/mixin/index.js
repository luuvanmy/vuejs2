export default{
    ...
    methods : {
        ...
        getObjectMetaValue(_object, meta_key, object_key){
            var val = '';
            if( _object.hasOwnProperty(meta_key) ){
                val = _object[meta_key] !== '' ?  _object[meta_key] : 
                    ( _object.hasOwnProperty( object_key ) ? _object[object_key] : '');
            }else{
                val = _object.hasOwnProperty( object_key ) ? _object[object_key] : '';
            }
            _object[meta_key] = val;
        },
        getSeoTitle( _object ){
            this.getObjectMetaValue(_object, 'meta_title', 'name');
            this.getObjectMetaValue(_object, 'meta_description', 'name');
            var metatag = null;
            ...
            if( ! metatag ){
                ...
                if(! metatag.meta_title || metatag.meta_title === '' ){
                    metatag.meta_title = _object.hasOwnProperty('meta_title') && _object.meta_title ? _object.meta_title : '';
                }
        ...
        incrementQuantity(product, increment = true, callback ){
            ...
            if( ! this.$store.state.allow_outofstock  ){
                var current_quantity = product.hasOwnProperty("quantity") && ! isNaN(product.quantity) ? parseFloat(product.quantity) : 0;
                if( current_quantity > product.available_quantity && product.manage_inventory == true ){
                    product.quantity = product.available_quantity;
                    this.showNotification('sản phẩm hiện đang hết hàng!!', 'error', 'danger' ); 
                }
            }
            // if(product.manage_inventory == false){
            //     this.$store.state.allow_outofstock = true;
            // }
            // if(product.manage_inventory == true){
            //     this.$store.state.allow_outofstock = false;
            // }
