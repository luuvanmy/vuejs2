            ...
            alias_pages : {
                ...
                "product" : {
                    resource_filters :  ['product_sort', 'product_view', 'product_perpage', 'sidebar_price', 'sidebar_property', 'sidebar_category','product_breadcrumb',
                        'product_detail_breadcrumb','product_page_title','product_tab']
                },
                "product_single":{
             ...
             async loadDefaultResources({ state,commit }){
               ...
               var res = await axios.post( state.api + 'page-data/resources', send_data, { headers : headers })
                ...
                if(res.success){
                    ...
                    /* xử lý trong trường hợp kiểm soát  sản phẩm */
                    if(state.default_data.hasOwnProperty('allow_outofstock')  ){
                        state.allow_outofstock = state.default_data.allow_outofstock.hasOwnProperty('allow_show') ? state.default_data.allow_outofstock.allow_show : true;
                    }
